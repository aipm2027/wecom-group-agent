...
# ── 转人工结构化原因（P1-3）────────────────────────────────────
# ┌─ 维护者注意 ─────────────────────────────────────────────┐
# │ 加/改关键词前先读 CLAUDE.md「加转人工关键词」菜谱与        │
# │ docs/技术文档/09 的 A3(政策名词≠诉求的坑)。标签值(REASON_*)│
# │ 被 evals 同源引用,改文案要连 golden.json 案例一起过评测。  │
# │ 改完必跑: make check + 在线双跑(evals --online --only esc)│
# └──────────────────────────────────────────────────────────┘
