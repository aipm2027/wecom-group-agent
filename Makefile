...
.PHONY: help test eval compile run-mock demo api docker-build docker-up docker-down docker-logs
...
compile: ## 全量字节编译 + import 冒烟(快速语法/依赖检查)
	$(PY) -m compileall -q core adapters tests evals main.py api_server.py examples
	$(PY) -c "import main, api_server; from adapters.wecom_kf import WecomKfAdapter; print('import 冒烟 OK')"
...
docker-up: ## 生产双服务起飞(agent + api,需 .env)
