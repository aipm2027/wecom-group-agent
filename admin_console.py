...
维护者注意:本文件 = 纯逻辑(ConsoleApp)+HTTP 薄壳+内嵌 HTML/JS(LOGIN_HTML/CONSOLE_HTML)。
- 改**界面文案/样式**:安全,直接改字符串里的中文/CSS,测试不锁文案;
- 改**JS 交互**:注意 api() 统一走 X-Requested-With 头,新增写操作必须复用它,否则被 CSRF 防线 403;
- 改**代理/鉴权逻辑**:先读 tests/test_console_offline.py 的断言,改完必跑 `python3 tests/test_console_offline.py`;
- 新增后端接口:加在 api_server.py(契约文档 docs/技术文档/06),本文件只做转发不做业务。
"""
